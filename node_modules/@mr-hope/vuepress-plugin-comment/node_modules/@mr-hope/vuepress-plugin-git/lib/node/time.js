...
const getCreatedTimeStamp = async (filePath) => {
    const { stdout } = await execa("git", [
        ...
        "--diff-filter=A",
        "--format=%at",
        (0, path_1.basename)(filePath),
    ], { cwd: (0, path_1.dirname)(filePath) });
...
const getUpdatedTimeStamp = async (filePath) => {
    const { stdout } = await execa("git", ["--no-pager", "log", "--format=%at", "-1", (0, path_1.basename)(filePath)], { cwd: (0, path_1.dirname)(filePath) });
    return Number.parseInt(stdout, 10) * 1000;
};
